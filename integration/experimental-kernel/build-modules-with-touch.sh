...
while IFS= read -r module; do
    ...
    name=${module//\//_}
    extra=()
    if [[ "$module" == qcom/opensource/touch-drivers ]]; then
        extra=(CONFIG_TARGET_PRODUCT_HOUJI=y CONFIG_ARCH_VOLCANO=n)
    fi
    if [[ "$module" == qcom/opensource/audio-kernel ]]; then
        extra=(CONFIG_SND_SOC_CS35L41=m TARGET_PRODUCT=lineage_houji)
    fi
    echo "BUILD $module"
    if make -C "$work/kernel/sm8650-modules/$module" \
        ...
        -j16 "${extra[@]}" > "$work/logs/modules/$name.log" 2>&1; then
...
done < "$work/modules-with-touch.txt"
